...
#define DOOM_TIC_TRACE 0
#if DOOM_TIC_TRACE
#define TIC_TRACE(...) printf(__VA_ARGS__)
#else
#define TIC_TRACE(...) do {} while (0)
#endif
...
static boolean BuildNewTic(void)
{
    ...
    TIC_TRACE("BuildNewTic: before I_StartTic\n");
    I_StartTic ();
    TIC_TRACE("BuildNewTic: after I_StartTic\n");
    TIC_TRACE("BuildNewTic: before ProcessEvents\n");
    loop_interface->ProcessEvents();
    TIC_TRACE("BuildNewTic: after ProcessEvents\n");

    // Always run the menu

    TIC_TRACE("BuildNewTic: before RunMenu\n");
    loop_interface->RunMenu();
    TIC_TRACE("BuildNewTic: after RunMenu\n");
    ...
    TIC_TRACE("BuildNewTic: before BuildTiccmd\n");
    loop_interface->BuildTiccmd(&cmd, maketic);
    TIC_TRACE("BuildNewTic: after BuildTiccmd\n");
...
void TryRunTics (void)
{
    ...
    TIC_TRACE("TryRunTics: enter singletics=%d gametic=%d maketic=%d ticdup=%d\n",
           singletics, gametic, maketic, ticdup);
    if (ticdup < 1)
    {
        ticdup = 1;
    }
    ...
    if (singletics)
    {
        ...
        TIC_TRACE("TryRunTics: singletics fast path\n");
        if (!BuildNewTic())
        {
            TIC_TRACE("TryRunTics: singletics BuildNewTic returned false\n");
            return;
        }

        lowtic = GetLowTic();
        TIC_TRACE("TryRunTics: singletics lowtic=%d gametic=%d\n",
               lowtic, gametic);

        if (!PlayersInGame())
        {
            TIC_TRACE("TryRunTics: singletics no players\n");
            return;
        }
        ...
        for (i=0 ; i<ticdup ; i++)
        {
            ...
            TIC_TRACE("TryRunTics: singletics before RunTic gametic=%d lowtic=%d\n",
                   gametic, lowtic);
            loop_interface->RunTic(set->cmds, set->ingame);
            TIC_TRACE("TryRunTics: singletics after RunTic gametic=%d\n", gametic);
            gametic++;
	...
	for (i=0 ; i<ticdup ; i++)
	{
            ...
            TIC_TRACE("TryRunTics: before RunTic gametic=%d lowtic=%d\n",
                   gametic, lowtic);
            loop_interface->RunTic(set->cmds, set->ingame);
            TIC_TRACE("TryRunTics: after RunTic gametic=%d\n", gametic);
	    gametic++;
